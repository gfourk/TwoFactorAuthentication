...
public class SC {
	...
	public SC(String my_name, String other_name, AESKey sessionKey, long last_received, long last_sent,int max_time){
		...
		this.other_name = other_name;
                ...
                this.KeySendEnc = new AESKey(crypt.hash(utils.concat(sessionKey.getEncoded(),("Enc "+my_name+" to "+other_name).getBytes())));
		this.KeyRecEnc = new AESKey(crypt.hash(utils.concat(sessionKey.getEncoded(),("Enc "+other_name+" to "+my_name).getBytes())));
                this.KeySendAuth = new AESKey(crypt.hash(utils.concat(sessionKey.getEncoded(),("Auth "+my_name+" to "+other_name).getBytes())));
                this.KeyRecAuth = new AESKey(crypt.hash(utils.concat(sessionKey.getEncoded(),("Auth "+other_name+" to "+my_name).getBytes())));

                this.max_time = max_time;
	}

	/****************************************************************************
	 * creates a MAC for the msg 
	 * as --Input: i || length(xi) || xi || mi--
	 * @param msg
	 * @return
	 * HMAC-SHA-256 (KeySendAuth, i || length(x) || x || m)
	 */
	private byte[] create_MAC(byte[] msg){
        ...
        private byte[] append_time(byte[] msg){
            byte[] time = utils.long_to_bytes(System.currentTimeMillis());
            return utils.concat(time, msg);
        }

        private boolean check_time(byte[] btime){
            return true;
            /*
            long time = utils.bytes_to_long(btime);
            if(time + this.max_time > System.currentTimeMillis())
                return false;
            else
                return true; */
        }
       ...
       try {
            res = utils.toHex(
                    crypt.AESEncrypt(
                                        msg_bytes,
                                        this.KeySendEnc,this.last_sent+1)
                                        );
        } catch (Exception ex) {
            return null;
        }
